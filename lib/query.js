 ...
 *
 * The following options are only for `findOne()` and `findById()`:
 *
 * - [rejectOnNotFound](./api.html#query_Query-rejectIfNotFound)
 *
...
/**
 * Sets the `rejectIfNotFound` option for `findById()` and `findOne()` methods.
 * When a callback method is not supplied, a promise is returned by 'Query.exec()', or Query behaves implicitly like a Promise for 'Query.then()' and 'Query.catch()'.
 * A promise for a single object can be considered to have failed if exactly one document is not found.  This option causes the promise to be rejected if these methods would return null.  'Query.catch()' can then be used to recover or re-throw from the failure to find exactly one thing.
 * This method is useful for constructing a replacement object, or for structuring a context-appropriate error message.
 *
 * Calling `query.explain(v)` is equivalent to `query.setOption({ explain: v })`
 *
 * ####Example:
 *  get an empty object rather than null so the promise chain can continue
 *
 *     const query = new Query();
 *     const res = await query.findOne({ a: 'email@example.com' }).rejectIfNotFound(true)
 *         .catch( () => ({}) )
 *         .then( doc => youDoYou(doc) )
 *     console.log(res);
 *
 *  report that the user was not found when no object with the email address is in the collection
 * 
 *     const email = 'email@example.com'
 *     const res = await query.findOne({ a: email }).rejectIfNotFound(true).catch( new Error('No user has email', email) )
 *
 * @param {Boolean} [reject] Whether to reject the promise rather than resolve with 'null'
 * @return {Query} this
 * @api public
 */

Query.prototype.rejectIfNotFound = function(reject = true) {
  this.options.rejectIfNotFound = reject;
  return this;
};
...
Query.prototype.findOne = function(conditions, projection, options, callback) {
  ...
  if (!callback) {
    ...
    const p = Query.base.findOne.call(this);
    if (options && options['rejectIfNotFound']) {
      return p.then( doc => { if ( doc === null ) { throw new Error('findOne: document not found'); } return doc; } );
    }
